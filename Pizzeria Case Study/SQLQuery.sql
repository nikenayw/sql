--Create a pizza_project database dan use it
create database pizza_project;
use pizza_project;

--Upload the table into azure data studio
--Show the table
...
select pizzas.size, count(order_details.order_id) as 'No of Orders', sum(order_details.quantity) as 'Total Quantity Ordered' 
from order_details
join pizzas on pizzas.pizza_id = order_details.pizza_id
